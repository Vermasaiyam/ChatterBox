...
const CommentDialog = ({ open, setOpen }) => {
    ...
    const sendMessageHandler = async () => {
        try {
            const res = await axios.post(`https://chatterbox-aaxc.onrender.com/api/post/${selectedPost._id}/comment`, { text }, {
                headers: {
                            ...
                            <Link to={`/profile/${selectedPost?.author?._id}`} className='flex gap-3 items-center'>
                                <div>
                                    <Avatar>
                                        ...
                                        <AvatarFallback><InitialsAvatar name={selectedPost?.author?.username || "CN"} className="h-full w-full flex items-center justify-center bg-slate-200 p-2 rounded-full" /></AvatarFallback>
                                    </Avatar>
                                </div>
                                <div>
